  ...
  const filtered = useMemo(() => {
    return payments
      .filter((p) => {
        ...
        if (filters.status !== "all" && String((p.paymentStatus || p.status || "").toLowerCase()) !== filters.status)
          return false;
          ...
          <div className="overflow-x-auto">
            <table className="table">
              ...
              <tbody>
                {isLoading ? (
                  <tr>
                    <td colSpan={tableColSpan} className="text-center py-6">
                      <Loader />
                    </td>
                  </tr>
                ) : filtered.length === 0 ? (
                  ...
                  paginatedData.map((p) => {
                    ...
                    if (!displayPurpose) {
                      displayPurpose = p.metadata?.issueId
                        ? "Boost"
                        : p.metadata?.userId
                ...
                {monthly.map((m) => {
                  ...
                  return (
                    <div key={m.month} className="flex-1 text-center">
                      <div
                        style={{ height: `${height}%` }}
                        className="bg-blue-500 rounded-t-md transition-all hover:bg-blue-600 cursor-pointer"
