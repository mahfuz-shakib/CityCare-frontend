...
import { Download, Zap, Star, TrendingUp, TrendingDown, CreditCard, MoreVertical } from "lucide-react";
...
const StatusChip = ({ status }) => {
  ...
  return (
    <span className="inline-flex items-center gap-1 text-[11px] font-bold px-2.5 py-0.5 rounded-full bg-red-100 text-red-600">
      FAILED
    </span>
  );
};
...
const downloadInvoicePDF = (payment) => {
  ...
  pdf.setFontSize(10);
  pdf.setTextColor(100, 100, 100);
  pdf.text("TOTAL AMOUNT", 30, y + 8);
  pdf.setFont("helvetica", "bold");
  pdf.setFontSize(18);
  pdf.setTextColor(22, 163, 74);
  pdf.text(
    `${payment.amount || payment.amount_total / 100 || 0} ${(payment.currency || "BDT").toUpperCase()}`,
    30,
    y + 20,
  );
  y += 35;
  ...
  const handleExportPDF = () => {
    ...
    autoTable(doc, {
      ...
      body: filtered.map((p, i) => {
        const purpose =
          p.purpose ||
          (p.metadata?.issueId ? "Priority Boost" : p.metadata?.userId ? "Premium Subscription" : "Unknown");
      ...
      <Container className="px-4 md:px-10">
        <div className="pt-8 pb-16 space-y-6">
          ...
          <PageHeader
            ...
            description="Review payment activity, subscription revenue, and issue boost transactions."
            actions={
              <div >
            ...
            {[
              {
                ...
                value: `tk ${totalRevenue.toLocaleString("en-BD", { minimumFractionDigits: 2 })}`,
                sub: null,
                trend: "+12.4%",
                trendUp: true,
                color: "bg-blue-600",
              },
              {
                ...
                value: `tk ${subRevenue.toLocaleString("en-BD", { minimumFractionDigits: 2 })}`,
                sub: `from ${subPayments.length} active members`,
                trend: "+8.1%",
                trendUp: true,
                color: "bg-indigo-500",
              },
              {
                icon: Zap,
                label: "Priority Boost Earnings",
                value: `tk ${boostRevenue.toLocaleString("en-BD", { minimumFractionDigits: 2 })}`,
                sub: `from ${boostPayments.length} issue boosts`,
                trend: "+24.0%",
                trendUp: true,
                color: "bg-violet-500",
              },
              ...
              >
                <div className="flex items-start justify-between mb-3">
                  ...
                  <span
                    className={`flex items-center gap-1 text-xs font-bold px-2 py-0.5 rounded-full ${c.trendUp ? "text-emerald-600 bg-emerald-50" : "text-red-500 bg-red-50"}`}
                  >
                      ...
                      const purpose =
                        ...
                        (p.metadata?.issueId
                          ...
                          : p.metadata?.userId
                            ...
                            : "Unknown");
                      const issueRef = p.metadata?.issueId
                        ? `#${String(p.metadata.issueId).slice(-4).toUpperCase()}`
                        : "";
                        ...
                        >
                          ...
                          <td className="px-5 py-4">
                            <div className="flex items-center gap-2.5">
                              <div
                                className={`w-8 h-8 rounded-full ${avatarColor(p.customerEmail)} flex items-center justify-center text-white text-xs font-bold shrink-0`}
                              >
                                {initials(p.customerEmail)}
                              </div>
                              <div>
                                <p className="font-semibold text-slate-800 text-xs">
                                  {p.customerEmail?.split("@")[0] || "Unknown"}
                                </p>
                          ...
                          <td className="px-5 py-4 font-bold text-slate-800">
                            {(p.amount || p.amount_total / 100 || 0).toLocaleString("en-BD", {
